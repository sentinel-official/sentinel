...
export async function checksentinelSocks(cb) {
    exec("net start sentinelSocksv101", function (stderr, stdout, error) {
        if (stderr && stderr.toString().trim().split(" ")[9] === 'already') {
            ...
            exec(`${username}\\AppData\\Local\\Sentinel\\app-0.1.01\\resources\\extras\\socks5\\service.exe`, function (execErr, execOut, execStd) {
                exec(`net start sentinelSocksv101`, function (stderr, stdout, error) {
                    cb(null);
                });
            });
        ...
        fs.readFile('resources\\extras\\socks5\\gui-config.json', 'utf8', function (err, conf) {
            if (err) { }
                ...
                fs.writeFile('resources\\extras\\socks5\\gui-config.json', config, function (writeErr) {
                    exec('net start sentinelSocksv101', function (servErr, serveOut, serveStd) {
                    })
                });
            }
        });
